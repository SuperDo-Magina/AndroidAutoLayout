...
public class AutoEditText extends EditText {
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.AutoEditText);

        int textSize = a.getInt(R.styleable.AutoEditText_auto_text_size, -1);
        if (textSize >= 0) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            float spacingExtra = a.getFloat(R.styleable.AutoEditText_auto_line_spacing_extra, -1);
        ...
        int pl = a.getInt(R.styleable.AutoEditText_auto_padding_left, 0);
        int pt = a.getInt(R.styleable.AutoEditText_auto_padding_top, 0);
        int pr = a.getInt(R.styleable.AutoEditText_auto_padding_right, 0);
        int pb = a.getInt(R.styleable.AutoEditText_auto_padding_bottom, 0);
        float ple = a.getFloat(R.styleable.AutoEditText_auto_padding_left_extra, 0);
        float pte = a.getFloat(R.styleable.AutoEditText_auto_padding_top_extra, 0);
        float pre = a.getFloat(R.styleable.AutoEditText_auto_padding_right_extra, 0);
        float pbe = a.getFloat(R.styleable.AutoEditText_auto_padding_bottom_extra, 0);
